    ...
    public View getView(final int position, View convertView, ViewGroup parent)
    {
        ...
        holder.BlockButton.setOnClickListener(new View.OnClickListener() {
            ...
            try {
                if (holder.BlockButton.getText().toString().equals("Block User"))
                {
                    ...
                    new WebHttpGetReq(context).execute("http://192.168.1.13:8080/Muaavin-Web/rest/Users/BlockUser?user_id=" + AesEncryption.encrypt(users.get(position).id)+"&isTwitterUser="+users.get(position).isTwitterUser);
                } else
                {
                    holder.BlockButton.setText("Block User"); users.get(position).state = "UnBlocked";
                    new WebHttpGetReq(context).execute("http://192.168.1.13:8080/Muaavin-Web/rest/Users/UnBlockUser?user_id=" + AesEncryption.encrypt(users.get(position).id)+"&isTwitterUser="+users.get(position).isTwitterUser);
                }
        ...
        holder.UnReport.setOnClickListener(new View.OnClickListener() {
            ...
            if (!users.get(position).state.equals("Blocked")){ // Can Only be UnReported if User is not Blocked
              try
              {
                new WebHttpGetReq(context).execute( "http://192.168.1.13:8080/Muaavin-Web/rest/Users/UnReportUser?user_id=" + AesEncryption.encrypt(users.get(position).id)+"&isTwitterData="+users.get(position).isTwitterUser);
              } catch (Exception e) {  e.printStackTrace(); }
